...
#import "UIButton+PBControl.h"
#import <objc/runtime.h>

@implementation UIButton (PBControl)

+ (void)load {
    Method originMethod = class_getInstanceMethod(self, @selector(sendAction:to:forEvent:));
    SEL originSEL = @selector(sendAction:to:forEvent:);
    Method replaceMethod = class_getInstanceMethod(self, @selector(pb_sendAction:to:forEvent:));
    SEL replaceSEL = @selector(pb_sendAction:to:forEvent:);
...
- (void)pb_sendAction:(SEL)action to:(id)target forEvent:(UIEvent *)event{
    ...
    if (needSendAction) {
        [self pb_sendAction:action to:target forEvent:event];
    }
}
